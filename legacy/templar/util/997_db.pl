...
$ARCONIC_DUNS   =       "961613887";    #Production. Alex Gerlants. 03/02/2020
...
# Alex Gerlants. 03/02/2020. Added ARCONIC block of code below.
if ($isa{Sndr} eq $ALCAN_DUNS) { 
...
} elsif ($isa{Sndr} eq $ARCONIC_DUNS) {
    $gs{FuncId}             = $data_nospaces[17];           # Functional ID
    $gs{Sndr}               = $data_nospaces[18];           # Application Sender Code
    $gs{Rcvr}               = $data_nospaces[19];           # Application Receiver Code
    $gs{Date}               = $data_nospaces[20];           # Data Interchange Date
    ...
    $gs{Version}            = $data_nospaces[24];           # Version
    $st{Type}               = $data_nospaces[25];           # Transaction Set ID
    $st{Control}            = $data_nospaces[22];           # Transaction Set Control Number
}
...
#print "isa{Sndr): $isa{Sndr)  st{Type}: $st{Type}  \n";

#Alex Gerlants. 03/02/2020. Added " || $isa{Sndr} eq $ARCONIC_DUNS"
if (($isa{Sndr} eq $ALCAN_DUNS || $isa{Sndr} eq $ALERIS_DUNS || $isa{Sndr} eq $ARCONIC_DUNS) && $st{Type} eq "997" ) {
