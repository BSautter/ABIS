...
print "Before IF . isa{Sndr}: $isa{Sndr}  ARCONIC_DUNS: $ARCONIC_DUNS  gs{FuncId}: $gs{FuncId}  st{Type}: $st{Type}  isa{TestIndc}: $isa{TestIndc}  isa{Rcvr}: $isa{Rcvr}  isa{Date}: $isa{Date}  index: $index  \n";
