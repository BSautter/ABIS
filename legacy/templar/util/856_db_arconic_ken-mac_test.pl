...
# $ARCONIC_DUNS	=	"005270160";         #Production
$ARCONIC_DUNS   =   "961613887";         #Production from Precision (PSI)
...
$message =~ s/\\/*/; # Replace "\" with "*" in the ISA line.

#print "message: $message   \n";
#print "======================================================\n";

$index = index ($message, "N1*MA");
# print "Position of 'N1*MA' in the string: $index\n";
...
# $message_nospaces =~ s/\\/*/; # Replace "\" with "*" in the ISA line.

# print "message: $message \n message_nospaces: $message_nospaces  \n"; 
...
if ($duns_new eq "ARNCFMW") {
  $duns_new = $ARCONIC_DUNS;
}

print "Before IF. duns_new: $duns_new   isa{Sndr}: $isa{Sndr}  ARCONIC_DUNS: $ARCONIC_DUNS  gs{FuncId}: $gs{FuncId}  st{Type}: $st{Type}  isa{TestIndc}: $isa{TestIndc}  isa{Rcvr}: $isa{Rcvr}  isa{Date}: $isa{Date}  index: $index  \n";

# if ($isa{Sndr} eq $ARCONIC_DUNS && $gs{FuncId} eq "SH" && $st{Type} eq "856" && $isa{TestIndc} eq "P" && $index gt 0) { # Production
# # if ($isa{Sndr} eq $ARCONIC_DUNS && $gs{FuncId} eq "SH" && $st{Type} eq "856" && $isa{TestIndc} eq "T" && $index gt 0) { # Development

if ($duns_new eq $ARCONIC_DUNS && $gs{FuncId} eq "SH" && $st{Type} eq "856" && $isa{TestIndc} eq "P" && $index gt 0) { # Production
# if ($duns_new eq $ARCONIC_DUNS && $gs{FuncId} eq "SH" && $st{Type} eq "856" && $isa{TestIndc} eq "T" && $index gt 0) { # Developme
...
#    print "Before # Grab the line for bol. segment: $segment  elements[3]: $elements[3]  hl_3: $hl_3  \n";
    ...
    if ($segment =~ /MEA/ and $elements[2] eq "N") {
            $netwt= $elements[3];
    }
...
#     }
    ...
    if ($segment =~ /N1/ and $elements[1] eq "MA") {
       $ship_from = $elements[4];
       $n1_ma_exists = 1;  
    }

#    print "Before if. segment: $segment  elements[1]: $elements[1]  n1_ma_exists: $n1_ma_exists   \n";

    if ($segment =~ /N1/ and $elements[1] eq "SF") {
#       print "Inside if. n1_ma_exists: $n1_ma_exists   \n";
    ...
    if ($segment =~ /N1/ and $elements[1] eq "ST") {
       ...
       #Insert into inbound_transaction and inbound_shipment
...
#    print "Inside  if ($segment =~ /MEA/. netwt: $netwt  grosswt: $grosswt  width: $width  lfeed: $lfeed  alloy: $alloy  gauge: $gauge  density: $density  \n";
    ...
    if ($segment =~ /REF/) {
        ...
        switch ($elements[1]) {
            ...
            case "C3"   {$material_num = $elements[2];
         }; # switch ($elements[1])
...
#print "Before Update inbound_coil table. po: $po  alloy: $alloy  temper: $temper  file_id: $file_id  bol: $bol  coil_id: $coil_id  ps_coil: $ps_coil  alloy: $alloy  temper: $temper   \n";
 ...
 if ( $ok_2send_997 = "Y" ) {
    # open(MAIL, "/usr/bin/mailx -s 'Arconic EDI 856 received. BOL ' . '$bol', 'agerlants@albl.com' < $INCOMING_COIL_856_X12/$X12_file|");

    $from = '856_3_db_arconic.pl';
    $to = 'agerlants@albl.com';
    $subject = 'EDI 856 received. BOL ' . $bol;
    $message = "EDI 856 file '" . $edi_file . "' received  successfully. " . "997 for file '" . $edi_file . "' created succsessfully.";
