...
#Alex Gerlants. 04/02/2019. Arconic EDI. Begin
# print "message: $message  \n";

my $pos_backslash = 0;
my $pos_tn = 0;
my $char_backslash = "\\";  # It is actually "\". Second "\" is escape.
my $char_tn = "2NDSFTPTN"; # Development       # 'TN' identifies Arconic TN
# my $char_tn = "039630926TTN"; # Production     # 'TN' identifies Arconic TN

$pos_backslash = index($message, $char_backslash);    # $result is 1 less than the actual position
$pos_tn = index($message, $char_tn);

# print "pos_backslash: $pos_backslash  char_backslash: $char_backslash pos_tn: $pos_tn  char_tn: $char_tn  \n";

if ( $pos_backslash gt 0 ) {
    $message =~ s/\\/*/; # Replace "\" with "*" on the ISA line.
   }

# print "message: $message  \n";
#Alex Gerlants. 04/02/2019. Arconic EDI. End
...
#print "elem: $elem  elemDelm: $elemDelm  sub: $sub  subDelm: $subDelm  seg: $seg  segDelm: $segDelm  \n";
...
$gs{Rcvr}       = $data_nospaces[18];       # Application Receiver Code #Alex Gerlants. 07/14/2023
$gs{Sndr}		= $data_nospaces[19];		# Application Sender Code
#$gs{Rcvr}		= $data_nospaces[20];		# Application Receiver Code #Alex Gerlants. 07/14/2023
...
#print "gs{FuncId}: $gs{FuncId}  st{Type}: $st{Type}  isa{Sndr}: $isa{Sndr}  isa{Rcvr}: $isa{Rcvr}  \n";
 print "isa{Sndr}: $isa{Sndr}  CONSTELLIUM_DUNS: $CONSTELLIUM_DUNS  gs{FuncId}: $gs{FuncId} st{Type}: $st{Type} isa{TestIndc}: $isa{TestIndc} gs{Control}: $gs{Control}  \n";
...
#Email from Troy Haag received on Fri 5/20/2022 4:29 PM
#Approval email from Lisa received on Mon 5/23/2022 10:10 AM.
#Change the last character on ISA line from "*" to "@"
...
open (FILE, ">${OUTGOING_997_X12}/$edi_file_997") or die " Can't create file !!";
	...
	#print	"GS*FA*$gs{Rcvr}*$gs{Sndr}*$gs_y_m_d*$isa_h_m*$gs_id*X*004010\n"; #Alex Gerlants. 07/14/2023
    print   "GS*FA*$gs{Sndr}*$gs{Rcvr}*$gs_y_m_d*$isa_h_m*$gs_id*X*004010~\n"; #Alex Gerlants. 07/14/2023
	print	"ST*997*0001~\n";
