...
# Version history
# File Name: 856_2_db_aleris.pl
# 07/01/2013	Victor Huang Initial Revison
...
if ($isa{Sndr} == $ALERIS && $gs{FuncId} == "SH" && $st{Type} == "856" ) {
...
$dbh->do("commit");
		...
		for $item_num ( 0 .. scalar (@hl_item) -1) {
			...
			@hl_item_mea1	=	split ( /$elemDelm/, $hl_item[$item_num][5] );
			@hl_item_mea2	=	split ( /$elemDelm/, $hl_item[$item_num][6] );
			@hl_item_mea3	=	split ( /$elemDelm/, $hl_item[$item_num][7] );
			@hl_item_mea4	=	split ( /$elemDelm/, $hl_item[$item_num][8] );
			@hl_item_mea5	=	split ( /$elemDelm/, $hl_item[$item_num][9] );
			@hl_item_ref1	=	split ( /$elemDelm/, $hl_item[$item_num][10] );
			@hl_item_ref2	=	split ( /$elemDelm/, $hl_item[$item_num][11] );
			@hl_item_ref3	=	split ( /$elemDelm/, $hl_item[$item_num][12] );
