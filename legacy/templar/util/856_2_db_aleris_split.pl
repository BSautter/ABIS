...
# Version history
# File Name: 856_2_db_aleris_split.pl
# 09/05/2002	Victor Huang Initial Revison
...
if ($isa{Sndr} == $ALERIS_DUNS && $gs{FuncId} == "SH" && $st{Type} == "856" ) {
...
foreach $st_line (@st) {
	...
	@st_dtm = split ( /$elemDelm/, $shipment[1] );
	@st_hls = split ( /$elemDelm/, $shipment[2] );
	@st_hls_td1 = split ( /$elemDelm/, $shipment[3] );
	@st_hls_td3 = split ( /$elemDelm/, $shipment[4] );
	@st_hls_ref = split ( /$elemDelm/, $shipment[5] );
	@st_hls_n11 = split ( /$elemDelm/, $shipment[6] );
	@st_hls_n12 = split ( /$elemDelm/, $shipment[7] );
	@st_hls_n13 = split ( /$elemDelm/, $shipment[8] );
	@st_hlo = split ( /$elemDelm/, $shipment[9] );
	@st_hlo_lin = split ( /$elemDelm/, $shipment[10] );
	@st_hlo_prf= split ( /$elemDelm/, $shipment[11] );
	@st_hlo_per1 = split ( /$elemDelm/, $shipment[12] );
	@st_hlo_per1 = split ( /$elemDelm/, $shipment[13] );


	$item_index = 0;
	for $i ( 14 .. scalar (@shipment)-3) {
		if ($shipment[$i] =~ /^HL\*/)  {
...
$dbh->do("commit");
		...
		for $item_num ( 0 .. scalar (@hl_item) -1) {
			...
			@hl_item_mea1	=	split ( /$elemDelm/, $hl_item[$item_num][1] );
			@hl_item_mea2	=	split ( /$elemDelm/, $hl_item[$item_num][2] );
			@hl_item_mea3	=	split ( /$elemDelm/, $hl_item[$item_num][3] );
			@hl_item_mea4	=	split ( /$elemDelm/, $hl_item[$item_num][4] );
			@hl_item_mea5	=	split ( /$elemDelm/, $hl_item[$item_num][5] );
			@hl_item_mea6	=	split ( /$elemDelm/, $hl_item[$item_num][6] );
			@hl_item_mea7	=	split ( /$elemDelm/, $hl_item[$item_num][7] );
...
open(MAIL, "/usr/bin/mailx -s 'Novelis EDI 856 received', 'vhuang\@albl\.com' < $INCOMING_COIL_856_X12/$X12_file|");
close(MAIL);
