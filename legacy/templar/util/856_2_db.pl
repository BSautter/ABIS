#! /usr/local/bin/perl -I/export/home/oracle9i/DBI-1.607/
...
# 05/28/2014    James Ni Update - added CASH date but no consumed ID
...
if ($isa{Sndr} eq $ALCAN_DUNS && $gs{FuncId} eq "SH" && $st{Type} eq "856" && $isa{TestIndc} eq "P" ) {
...
#				print "COIL: $coil_id  PACK: $pack_id NETWT:$netwt Width: $width Lfeed: $lfeed Gauge: $gauge Alloy: $alloy Density: $density Temper: $temper\n";
				...
				$dbh->do(" INSERT INTO inbound_coil(EDI_FILE_ID, BOL, ITEM_NUM, COIL_NUMBER, PS_COIL_NUMBER, NET_WEIGHT, GROSS_WEIGHT, LINEAL_FEED, COIL_WIDTH, COIL_GAUGE, LOT, PACK_ID, DENSITY, ALLOY, TEMPER, MATERIAL_NUM, CASH_DATE
						)
                                         VALUES ( $file_id, 
                                                '$bol', 
						 $item_num, 
