...
while ( my $x = <FILE> ) { $message .= $x; }
close(FILE);

# Set-up delimiters within the EDI data
	my $elem = substr( $message, 103, 1 );
	my $elemDelm = quotemeta $elem;		# Element Separator
	my $sub = substr( $message, 104, 1 );
	my $subDelm = quotemeta $sub;			# Sub Element Separator
	my $seg = substr( $message, 105, 1 );
	my $segDelm = quotemeta $seg;			# Segment Separator
...
my @data_seg = split ( /$segDelm/, "$message" );
my @st;
...
if ( $gs{FuncId} eq "RT" && $st{Type} eq "863" ) { # Start of 863 if-statement

#print "st_index: $st_index\n";
# Create 997 for this 863 transaction

my $sth3 = $dbh->prepare ( "SELECT edi_gs_log_seq.NEXTVAL from dual");
$sth3->execute;
my $gs_id = $sth3->fetchrow();
my $edi_file_997 = "$isa{Control}.edi";

#print "isa{Control}: $isa{Control}  \n";

#$year4 = $year+1900;
#$year2 = sprintf ("%02d", $year % 100);
my $isa_y_m_d = sprintf ("%02d%02d%02d", $year % 100, $mon+1, $mday);
my $isa_h_m = sprintf ("%02d%02d", $hour, $min);
my $gs_y_m_d = sprintf ("%d%02d%02d", $year+1900, $mon+1, $mday);
my $isa_id = sprintf("%09d", $gs_id);

#print "gs{Rcvr}: $gs{Rcvr}  gs{Sndr}: $gs{Sndr}  gs{Control}: $gs{Control}  st_index: $st_index  gs_id: $gs_id  isa_id: $isa_id  \n";
