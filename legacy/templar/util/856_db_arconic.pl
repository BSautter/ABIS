...
if ($isa{Sndr} eq $ARCONIC_DUNS && $gs{FuncId} eq "SH" && $st{Type} eq "856" && $isa{TestIndc} eq "P" && $index lt 0) { # Production
# if ($isa{Sndr} eq $ARCONIC_DUNS && $gs{FuncId} eq "SH" && $st{Type} eq "856" && $isa{TestIndc} eq "T" && $index lt 0) { # Development
