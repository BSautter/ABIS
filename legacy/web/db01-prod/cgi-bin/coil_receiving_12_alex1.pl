...
#Arconic-TN
&addqrcode('43450J','(                            )  68281950-36000-19177CDD0E   8L172213466501384113980.90$00000000008$00000000007$');

#Novelis-Oswego
# &addqrcode('1721975','04  7000016888 X610-18314  OSW   17219750247900245451880137051.100275BU$0002450019441X$');
exit(0);
...
sub addqrcode  {
    ...
    #Alex Gerlants. 08/01/2019. Begin

    my $first_character_qr = substr($coil_qr_code, 0, 1);

    print "first_character_qr: $first_character_qr  \n";

=pod
=cut

    if ($first_character_qr ne '(') { #Novelis...for now 
       $coil_org_num_count = () = $coil_qr_code =~ /$coil_org_num/g; #Original statement
    } else {  #Arconic-TN
       ...
       print "Before select ...coil_org_num: $coil_org_num \n";
...
=pod
=cut
       # $lot = "7-L17218-31E"; #TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY

       print "Before coil_org_num_count =.  coil_qr_code: $coil_qr_code\nlot: $lot  \n";


       #Remove the dashes from $lot. Start
       #----------------------------------
       my $lot_temp = "";
       my $start_pos = 0;
       my $length_of_lot = length($lot);
       my $char_value = "";

       # $lot = "8L17221"; #TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY

       while ( $start_pos <= $length_of_lot ) {
          $char_value = substr($lot, $start_pos, 1);

          print "char_value: $char_value  \n";

          if ( $char_value ne "-" ) {
              $lot_temp = $lot_temp . $char_value;
              print "    lot_temp: $lot_temp  \n";
          }
          $start_pos++
       }

       print "After LOOP. lot: $lot  lot_temp: $lot_temp  \n";

       $lot = $lot_temp;
       #----------------------------------
       #Remove the dashes from $lot. End


       $coil_org_num_count = () = $coil_qr_code =~ /$lot/g;

       print "After coil_org_num_count =.  coil_org_num_count: $coil_org_num_count  \n";

    }

    #Alex Gerlants. 08/01/2019. End

        # my $coil_org_num_count = () = $coil_qr_code =~ /$coil_org_num/g; #Alex Gerlants. 08/01/2019. Comment out

 print "Before if ( (length(coil_qr_code) > 67. substr: $substr  coil_org_num_count: $coil_org_num_count  \n";
...
<body>
ENDOFTEXT
        print '<b>Invalid QR Code. Please re-scan. lot: $lot</b>';
print <<ENDOFTEXT;
        <FORM>
                <INPUT type="button" value="OK" onClick="history.back()">
        </FORM>
