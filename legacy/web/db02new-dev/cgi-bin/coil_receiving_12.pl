...
#Alex Gerlants. 11/29/2022. IT Request 1737. Added "  && (substr($coil_qr_code, 1, 2) eq "04")  "
#Alex Gerlants. 11/29/2022. IT Request 1737. Changed from "> 67" to "> 71"

print "coil_qr_code: $coil_qr_code  \n";

 if ( (substr($coil_qr_code, 1, 2) eq "04") && (length($coil_qr_code) > 71) && (index($coil_qr_code, $substr) != -1) && (length($coil_org_num) > 2) && $coil_org_num_count == 1) {
#if ( (length($coil_qr_code) > 71) && (index($coil_qr_code, $substr) != -1) && (length($coil_org_num) > 2) && $coil_org_num_count == 1)  && (substr($coil_qr_code, 1, 2) eq "04") {
