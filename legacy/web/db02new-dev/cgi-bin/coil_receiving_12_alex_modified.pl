...
# $db       = "dbi:Oracle:host=db01;sid=abc11; port=1523"; #Alex Gerlants. 07/30/2019. Comment out

# $db='dbi:Oracle:host=192.168.1.9;sid=abc11;port=1523';           #Production #Alex Gerlants. 07/30/2019
$db='dbi:Oracle:host=192.168.1.11;sid=abc11;port=1523';           #Development #Alex Gerlants. 07/30/2019
...
# print "coil_org_qr_code: $coil_org_qr_code  coil_org_num_data: $coil_org_num_data  \n";
# &addqrcode($coil_org_num_data,$coil_org_qr_code);

# Arconic. Lot is NOT in QR string. Invalid QR Code message
# &addqrcode('86605I','()    B7581VBB6000T19190CDD0E   L176161270141504098160.90$04719048536$');

# Arconic. Lot is in QR string. Valid QR Code
&addqrcode('86605I','()    B7581VBB6000T19190CDD0E0-K62808-1270141504098160.90$04719048536$');

# Novelis
# Invalid QR string
# &addqrcode('1714113','03  7000012170 X610T18144  OSW   17141140298860294081800128630.900285BU$0004800059001$0254500298901$');

# Valid QR string
# &addqrcode('1714113','03  7000012170 X610T18144  OSW   17141130298860294081800128630.900285BU$0004800059001$0254500298901$');
...
my @params = param();
print "After Switch. param('coil_org_num') \n";
...
# my $l_qr_code = length($coil_qr_code);
# my $i = index($coil_qr_code, $substr);
# my $l_coil_org_num = length($coil_org_num);
    my $first_character_qr = substr($coil_qr_code, 0, 1); #Alex Gerlants. 07/30/2019

# print "coil_org_num: $coil_org_num  coil_qr_code: $coil_qr_code  substr: $substr  \n";
# print "l_qr_code: $l_qr_code  i: $i  l_coil_org_num: $l_coil_org_num  first_character_qr: $first_character_qr  \n";

if ($first_character_qr ne '(') { #Novelis...for now #Alex Gerlants. 07/30/2019
   $coil_org_num_count = () = $coil_qr_code =~ /$coil_org_num/g;

#Alex Gerlants. 07/30/2019. Begin
} else { #Arconic-TN
   #Coil received either from Arconic-TN directly, or from Precision (PSI) after slitting
   #Arconic-TN does not send us coil_org_num in QR barcode string.
   #Thus I am using LOT instead to check if this QR code string belongs to this coil_org_num

   $ENV{ORACLE_HOME} = '/apps/oracle/product/9.2.0.1.0';
   $dbh = DBI->connect( $db, $username . '/' . $passwd ) || error_report("Error connecting to database\n");

   my $sth = $dbh->prepare(
          "select inbound_coil.lot
           from   inbound_coil
           where  inbound_coil.coil_number = '$coil_org_num'
           and    rownum = 1"
   );

   $sth->execute() || error_report("Error while executing SQL statement from INBOUND_COIL.\n");
   $sth->bind_col(1, \$lot);
   ...
   print "Inside ELSE first_character_qr. db: $db \n sth: $sth \n coil_org_num: $coil_org_num  lot: $lot  \n";
#   $coil_org_num_count = 1;
$coil_org_num_count = () = $coil_qr_code =~ /$lot/g;
}
#Alex Gerlants. 07/30/2019. End

print "coil_org_num_count: $coil_org_num_count  \n";

# if ( (length($coil_qr_code) > 67) && (index($coil_qr_code, $substr) != -1) && (length($coil_org_num) > 2)) {
if ( (length($coil_qr_code) > 67) && (index($coil_qr_code, $substr) != -1) && (length($coil_org_num) > 2) && $coil_org_num_count == 1)
{
        print "inside IF length($coil_qr_code)  \n";
